...
This script works by generating all possible reads of a given size from a reference genome or chromosome, simply by moving a window of that size on base at a time over each chromosome.
Those reads are then mapped to the entire reference genome using Bowtie (the option to use your own choice of aligner or choose different Bowtie options will be added in the future).
Finally, any reads which either don't map anywhere (because of the presence of Ns in the reads) or that map to more than one place are detected, and the program return a mapability file
in the bed format.
'''
...
#The name of the final mapability file: if name of input file is x.fa or x.fasta, will be x_window_{window_size}_mapability.bed, otherwise will be full input filename followed by _window_{window_size}_mapability.bed
...
for chromosome in ref_genome:
	...
	read_out = chromosome.id + '_'.join(str(datetime.datetime.now()).split()) + '_all_possible_reads.temp'
	sam_out =  chromosome.id + '_'.join(str(datetime.datetime.now()).split()) + '_all_possible_reads_aligned.sam.temp'
	...
	print subprocess.check_output('bowtie2 -f -k 2 -x %s -U %s -S %s' %(args.reference_index, read_out, sam_out), shell = True),
